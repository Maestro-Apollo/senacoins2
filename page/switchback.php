...
class switchClass extends database
{
    ...
    public function switchFunction()
    {
        ...
        if ($res2) {
            ...
            $sqlName = "select * from premium_user where username = '$user' ";

            $resName = mysqli_query($this->link, $sqlName);
            if (mysqli_num_rows($resName) > 0) {
                echo "Already In Premium Account";
            } else {
                $sql4 = "DELETE from standard_user where username = '$user' ";
                mysqli_query($this->link, $sql4);

                $sql3 = "INSERT INTO `premium_user` (`id`, `fname`, `lname`, `username`, `email`, `password`, `address1`, `address2`, `country`, `state`, `zip`, `image`) VALUES (NULL, '$fname', '$lname', '$user', '$email', '$password', '$address1', NULL, '$country', '', '', '$image')";
                mysqli_query($this->link, $sql3);



                header('location:standard_users_list.php');
            }
        } else {
            echo "Already in Premium Account";
        }
